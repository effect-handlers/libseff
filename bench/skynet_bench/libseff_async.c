...
void async_schedule(
    ...
    queue_push(&self->workers[0].task_queue, task);
}

task_t *try_get_task(worker_thread_t *self) {
    task_t *own_task = queue_pop(&self->task_queue);
    ...
    for (size_t i = 1; i < self->scheduler->n_workers; i++) {
        task_t *stolen_task =
            queue_steal(&scheduler->workers[(worker_id + i) % n_workers].task_queue);
    ...
    while (true) {
        ...
        if (current_task->coroutine->state == FINISHED) {
            ...
            SPINLOCK(promise, {
                ...
                if (waiter != NULL) {
                    queue_push(&self->task_queue, waiter);
                }
            });
            ...
            switch (request->id) {
                CASE_EFFECT(request, await, {
                    ...
                    SPINLOCK(awaited, {
                        ...
                        if (payload.fut->state == READY) {
                            queue_push(&self->task_queue, current_task);
                        } else {
                ...
                CASE_EFFECT(request, async, {
                    ...
                    queue_push(&self->task_queue, new_task);
                    queue_push(&self->task_queue, current_task);
                    break;
                });
            default:
